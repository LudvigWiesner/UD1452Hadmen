...
State Game::update()
{
	...
	while (elapsedTimeSinceLastUpdate > timePerFrame)
	{
		...
		{
			waterPlant->setFourPoints(waterPurifyCords);

			for (int y = waterPurifyCords[2]; y < waterPurifyCords[3]; y++)
			{
				for (int x = waterPurifyCords[0]; x < waterPurifyCords[1]; x++)
				{
					TileEntity* temp = tileMap->getTile(x, y);
					if(temp.)
				}
			}
		}
